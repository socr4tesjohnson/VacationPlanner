...
async function test() {
  try {
    ...
    const test2 = await prisma.contactInquiry.update({
      where: { id: "6c19d4f9-0683-4ac2-9e27-5c2b465e83c2" },
      data: { quotedAmount: new Prisma.Decimal(12500.5) },
    });
    ...
    console.log(
      "Success! CommissionAmount:",
      test5.commissionAmount?.toString()
    );
    console.log("");
    ...
    console.log(
      "Success! Checklist:",
      test6.checklist?.substring(0, 50) + "..."
    );
    console.log("");
    ...
    console.log(
      "- checklist:",
      final.checklist
        ? "Present (" + final.checklist.length + " chars)"
        : "null"
    );
  } catch (error) {
