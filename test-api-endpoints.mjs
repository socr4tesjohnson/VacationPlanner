...
async function testFeature1_PrivateNotes(inquiryId) {
  ...
  try {
    ...
    const getResponse = await fetch(
      `${BASE_URL}/api/admin/inquiries/${inquiryId}`,
      {
...
async function testFeature2_QuoteCommission(inquiryId) {
  ...
  try {
    ...
    console.log(
      "Step 1: Saving quote with $5000 quoted amount and 15% commission..."
    );
    ...
    console.log(
      ...
      patchResult.inquiry?.quotedAmount,
      typeof patchResult.inquiry?.quotedAmount
    );
    console.log(
      "Response commissionRate:",
      patchResult.inquiry?.commissionRate,
      typeof patchResult.inquiry?.commissionRate
    );
    console.log(
      "Response commissionAmount:",
      patchResult.inquiry?.commissionAmount,
      typeof patchResult.inquiry?.commissionAmount
    );
    ...
    const getResult = await getResponse.json();
    console.log(
      "Persisted quotedAmount:",
      getResult.inquiry?.quotedAmount,
      typeof getResult.inquiry?.quotedAmount
    );
    console.log(
      "Persisted commissionRate:",
      getResult.inquiry?.commissionRate,
      typeof getResult.inquiry?.commissionRate
    );
    console.log(
      "Persisted commissionAmount:",
      getResult.inquiry?.commissionAmount,
      typeof getResult.inquiry?.commissionAmount
    );
    ...
    const passed =
      valuesPersisted && correctTypes && patchResponse.status === 200;
...
async function testFeature3_Checklist(inquiryId) {
  ...
  try {
    ...
    console.log(
      "Initial checklist:",
      getResult.inquiry?.checklist || "(empty)"
    );
...
async function main() {
  ...
  try {
    ...
    if (inquiries.length === 0) {
      console.error(
        "❌ No inquiries found in database. Please create test data first."
      );
      return;
    }
